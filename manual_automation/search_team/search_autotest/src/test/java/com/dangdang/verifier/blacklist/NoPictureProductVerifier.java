...
public class NoPictureProductVerifier{
	...
	public static boolean doVerify(FuncQuery query) {
		...
		//无图片的商品位置，初始为1，没迭代一次加1，最后的值即使无图片的商品位置
		int no_image_count=1;
		...
		while(iterator.hasNext()){
			...
			if(numberImages.equals("0")){
				...
				if(positionMap.size()==0){
					positionMap.put(no_image_count+"", no_image_count);
					break;
				}
			}
		}
		...
		if(positionMap.size()==1&&iterator.getTotalCount()>0){
			String positon=positionMap.get(positionMap.keySet().iterator().next()).toString();
			avgPositon = Double.parseDouble(positon)/iterator.getTotalCount();
		}
