...
public class NoUMSeachVerifierScheduler {
	...
	public void noUMSeachVerifier() {
		...
		warnContent.append("<html><head><meta http-equiv=Content-Type content='text/html; charset=utf-8'></head><body><table border=1 cellspacing=0 cellpadding=0><tr><th>功能模块</th><th>通过query</th><th>失败query</th><th>总计</th><th>跳过率</th><th>实际通过率</th><th>预期通过率</th></tr>");
...
//						logger.debug("URL - "+url);
		...
		content.append(String.format("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>", 
				"无UM搜索NoUMSeachVerifierScheduler", pass, fail, "0", pass+fail, d3));
		...
		if(actualPassrate < expectedPassrate){
			warnContent.append(String.format("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>", 
					"无UM搜索:"+NoUMSeachVerifierScheduler.class.getName(), pass, fail, pass+fail+0, "0.00%", actualPassrate + "%", expectedPassrate + "%"));			
