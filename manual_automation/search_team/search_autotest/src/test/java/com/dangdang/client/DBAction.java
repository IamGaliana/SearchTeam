...
public class DBAction {
	...
	// added by 高彦君 @ 205/06/04
	private static final String MIN_PASSRATE = "min_passrate";
	private static final String MAX_SKIPRATE = "max_skiprate";
	 ...
	 * @author dongxiaobing
	 * @return
	 * @func:get product id from table search_product_id
	 */
	public List<FuncQuery> getPorductId(){
		List<FuncQuery> list = new ArrayList<FuncQuery>();
		Connection conn = DBConnUtil.getConnection();
		StringBuffer buffer = new StringBuffer();
		buffer.append("SELECT * ");
		buffer.append("FROM search_product_id ");
		buffer.append("WHERE "); 
		...
		try {
			while(result.next()){
				...
				query.setProduct_id(result.getString(PRODUCTID));
				list.add(query);
			}
			return list;
	...
	public List<FuncQuery> getFuncQuery(){
		...
		try {
			while(result.next()){
				...
				query.setExpect(result.getString(EXPECT));
				query.setProduct_id(result.getString(PRODUCTID));
				query.setTest_data(result.getString(TEST_DATA));
				list.add(query);
			}
			return list;
		} catch (SQLException e) {
	...
	public List<FuncVP> getFVP(){
		...
		try {
			while(result.next()){
				...
				// added by 高彦君 @ 205/06/04
				fvp.setMinPassrate(Double.valueOf(result.getString(MIN_PASSRATE)));
				fvp.setMaxSkiprate(Double.valueOf(result.getString(MAX_SKIPRATE)));
				list.add(fvp);
			}
			return list;
		} catch (SQLException e) {
