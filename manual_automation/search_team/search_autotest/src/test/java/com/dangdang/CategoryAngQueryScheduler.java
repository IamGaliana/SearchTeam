...
public class CategoryAngQueryScheduler {
	...
	private int doQuery(FuncCatePath category, FuncVP fvp) {
		...
		logger.info(String.format(" - [LOG_SUMMARY] - categoryname: %s, query: %s", category.getFcate_name()+category.getCat_path(), category.getQuery()));
		...
		if (l_iterator.getTotalCount()<1||s_iterator.getTotalCount()<1) {
			logger.info(String.format(" - [LOG_SKIP] - no result:categoryname: %s, query: %s", category.getFcate_name()+category.getCat_path(), category.getQuery()));
			return -2;
		}
		
		if(doVerify(l_iterator, s_iterator, fvp, infop)){
			logger.info(" - [LOG_SUCCESS] - verify function 【"+fvp.getFvpname()+"】 for category: "+category.getFcate_name()+category.getCat_path()+"query:"+category.getQuery());
			return 0;
		}else{
