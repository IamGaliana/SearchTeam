...
//		Object[] categorys = dataMap.keySet().toArray();
		...
		if (iterator.getTotalCount() < 1) {
			...
			for (int i = 0; i < 600; i++) {
				...
				//if (AssertTools.singleTopStatus(XMLParser.product_scope(node))) {
					//logger.info("单品置顶ID："+XMLParser.product_id(node));
				//}
		...
		for (Entry<Integer, List<Node>> entry : resultmap.entrySet()) {
			...
			for(Node nodeV : entry.getValue()){
				...
				pidStrings.add(XMLParser.product_id(nodeV)+
						(AssertTools.singleTopStatus(XMLParser.product_scope(nodeV))?"(单)":"")
						+ (isvalidProd(entry.getKey(), nodeV)?"(新)":"(旧)"));
			}
	...
	private boolean validList(Integer integer, List<Node> value) {
		if (value.size()==0) {
			logger.info(" - 该分类无商品: " + categorys[integer]);
			return true;
		}else {
			int k = 0;//记录新品数
			int n = 0;//置顶单品数	
			...
			for (int m = 0; m < value.size(); m++) {	
				if (AssertTools.singleTopStatus(XMLParser.product_scope(value.get(m)))) {
					n++;
				}else {
					for (int j = m + 1; j < value.size(); j++) {
						if (AssertTools.singleTopStatus(XMLParser.product_scope(value.get(j)))) {							
							logger.info(" - 应该置顶单品未被置顶: "+XMLParser.product_id(value.get(j)));
							return false;
						}
					}
				}
			}
			logger.info("相关配置分类"+categorys[integer]+"下前"+n+"个商品为单品置顶");
			
			// 验证新品加权是否生效，顺序如果为“新-旧-新”则有错误
			for (int i = 0; i < value.size(); i++) {	
				if(AssertTools.singleTopStatus(XMLParser.product_scope(value.get(i)))){
					continue;
				}else if(isvalidProd(integer, value.get(i))) {
					k++;
					continue;
				}else {
					for (int j = i; j < value.size(); j++) {
						// 如果是单品置顶的品，旧品也不予考虑；如果非单品置顶且“新旧新”，那么就有错
