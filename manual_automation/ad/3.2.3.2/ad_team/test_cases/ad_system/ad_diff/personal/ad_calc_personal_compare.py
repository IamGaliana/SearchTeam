...
url_ilst = []
tables = excel_table_byindex('personal_url_for_compare.xlsx', by_index= 0)
for row in tables:
    url_ilst.append(row["url"])

title = time.strftime('%Y-%m-%d_%H.%M.%S_',time.localtime(time.time()))
test_report = u"\\\\ddfile\\技术部\\测试部\\广告系统\\自动化测试报告\\personal\\"+title+u"ad_calc_personal_compare.html"
test_log = u"\\\\ddfile\\技术部\\测试部\\广告系统\\自动化测试报告\\personal\\"+title+u"ad_calc_personal_compare.txt"
f = file(test_log.encode('GBK'), 'a')

@ddt
class ad_calc_personal(unittest.TestCase):
    def setUp(self):
        self.URL_OLD = "http://10.255.242.162:9090%s"
        self.URL_NEW = "http://10.255.242.213:9090%s"
