...
query_list_nbook = []
tables = excel_table_byindex('keyword_for_compare.xlsx', by_index = 1)   #读取第二个sheet的数据
for row in tables:
    query_list_nbook.append(row["keyword"].encode("utf-8"))

query_list_null = []
tables = excel_table_byindex('keyword_for_compare.xlsx', by_index = 2)  ##读取第三个sheet的数据
for row in tables:
...
test_report = u"\\\\ddfile\\技术部\\测试部\\广告系统\\自动化测试报告\\search\\"+title+u"ad_calc_search_nbook&null_compare.html"
test_log = u"\\\\ddfile\\技术部\\测试部\\广告系统\\自动化测试报告\\search\\"+title+u"ad_calc_search_nbook&null_compare.txt"
...
class ad_calc_search(unittest.TestCase):
    def setUp(self):
        self.URL_NEW = "http://10.255.242.162:8080/getAds/cpc/0/0/100/%s/184537119/0/0/0/107169205/%s/-/0/0/0/debug"
        self.URL_OLD = "http://10.255.242.213:8080/getAds/cpc/0/0/100/%s/184537119/0/0/0/107169205/%s/-/0/0/0/debug"
        pass
    @data(*query_list)
    def test_case(self, query):
        ...
        if value.startswith("01") == True and not value.endswith(".00.00.00.00.00"):
