...
public class SearchRankingBlackListVerifier{
	...
	// 日志记录器
	private static Logger logger = Logger.getLogger(SearchRankingBlackListVerifier.class);
	...
	public boolean doVerify(BlackListQuery queryInfo) {
	    try {
		    ...
		    if(start_date.before(now)&&end_date.after(now)){//黑名单生效
		    	logger.fatal("blacklist filer query ["+queryInfo.getQuery()+"] is on");
		    	return isBlackList(queryInfo,true);
		    }else{//黑名单不生效
		    	logger.fatal("blacklist filer query ["+queryInfo.getQuery()+"] is off");
