...
public class CategoryDirectBlacklistScheduler {
	...
	public void CategoryDirectBlacklist() { 
			...
			content.append(String.format("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>", 
					"分类直达黑名单CategoryDirectBlacklistScheduler", pass, fail, 0, pass+fail+0, d3));
			...
			if(pr < 100){
				warnContent.append(String.format("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>", 
						"分类直达黑名单CategoryDirectBlacklistScheduler", pass, fail, 0, pass+fail, "0.00%", pr + "%", "100.00%"));
	...
	public void CategoryFeedbackFilter() { 
			...
			content.append(String.format("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>", 
					"分类反馈过滤机制CategoryDirectBlacklistScheduler", pass, fail, 0, pass+fail+0));
			logger.info("Result  ####    Passed Count:"+pass+" Failed Count:"+fail);
			double pr = Calculator.passrate(pass, pass+fail);
			double fr = 100.00 - pr;			
			logger.info("Result  ####    Passed Rate:"+ pr+"% Failed Rate:"+ fr+"%");
			...
			if(pr < 100){
				warnContent.append(String.format("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>", 
						"分类反馈过滤机制CategoryDirectBlacklistScheduler", pass, fail, 0, pass+fail, "0.00%", pr + "%", "100.00%"));
				warnContent.append("</table></body></html>"); 
				Utils.sendWarningMail(warnSubject, warnContent.toString(), "HTML");
			}
			long d2 = System.currentTimeMillis();
