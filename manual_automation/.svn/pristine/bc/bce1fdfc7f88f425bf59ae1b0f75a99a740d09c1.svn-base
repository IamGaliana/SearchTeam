...
public class TestLauncher {
	...
	public int doQuery(String vps, String query, String medium){
		...
		//获取默认的查询内容	
		Map<String, String> infop = null;
		try{
			infop = URLBuilder.getPreSearchInfo(query);  
		}catch(Exception ex){
			logger.error(" - [Exception in presearch] - verify function 【"+ vps+"】 for query: "+query + ", exception is:" + ex.getMessage());
			return -1;
		}
		if(infop == null || infop.isEmpty()){
		...
		// 如果是翻页验证，要取得所有品（假设有100000个），而不是默认的1200个
		if(vps.equals("page_turn"))
			iterator = new ProdIterator(urlp,100000);	
		else
			iterator = new ProdIterator(urlp);
	...
	public int l_doQuery(String vps, String catepath, String catename){
		...
		Map<String, String> infop = null;
		try{
			infop = URLBuilder.l_getPreSearchInfo(catepath);  //获取默认的查询内容
		}catch(Exception ex){
			logger.error(" - [Exception in presearch] - verify function 【"+ vps+"】 for cat_path: "+catepath+", exception is: "+ex.getMessage());
			return -1;
		}
...
//		}
	...
	public void start(String fc, String fvpc,String flag){
		...
		if(fvps.size()>0){						
			for(FuncVP fvp : fvps){
				...
				// 增加脚本低通过率预警邮件内容，计算实际通过率，与fvp.minpassrate比较后决定是否发送邮件，modified by 高彦君 @ 205/06/04 
				double actualPassrate = Calculator.passrate(passed+skiped,passed+failed+0);
				double expectedPassrate = fvp.getMinPassrate();
	...
	public int doQuery(String vps, String query, String medium,FuncQuery queryInstance){
		...
		Map<String, String> infop = null;
		try{
			infop = URLBuilder.getPreSearchInfo(query);  //获取默认的查询内容	
		}catch(Exception ex){
			logger.error(" - [Exception in presearch] - verify function 【"+ vps+"】 for query: "+query+", exception is: "+ex.getMessage());
			return -1;
		}
	...
	/**
	 * 回归测试方法入口, 和常规方法不同的是，调用的doQuery方法是以FuncQuery和FuncVP整个对象为参数的,且多用于对比包含不同参数url返回的两个结果集
	...
	public int check_VPS_infop(String vps,Map<String, String> infop,String query){
		if ((infop.containsKey("totalCount") && Integer.valueOf(infop.get("totalCount"))<1) 
				|| (infop.containsKey("productCount") && Integer.valueOf(infop.get("totalCount")) < 1)){
