...
test_report = REPORT_PATH + title + u"ad_calc_search_nbook&null_compare.html"
test_log = LOG_PATH + title + u"ad_calc_search_nbook&null_compare.txt"
...
class ad_calc_search(unittest.TestCase):
    ...
    def test_case(self, query):
        ...
        #print "search=0:", old_result_search_type_0
        ...
        #print "search=1:", old_result_search_type_1

        old_result_search_type_3 = data_filter(old_result, "search_type", "-1")  #老版本过滤了search_type=-1的数据。 也就是相关性匹配的数据
        new_result_search_type_3 = data_filter(new_result, "search_type", "-1")  #新版本过滤了search_type=-1的数据。 也就是相关性匹配的数据

        #print "search=-1:", old_result_search_type_3
        ...
        old_list_xiangguan = get_ad_id_set_list_by_bidakscore(old_result_search_type_3)
        new_list_xiangguan = get_ad_id_set_list_by_bidakscore(new_result_search_type_3)
        ...
        for i in range(0, len(new_list_xiangguan)-1):
            f.write("第%s个相关性匹配广告set\n" % str(i))
            f.write("old_list:%s\n" % old_list_xiangguan[i])
            f.write("new_list:%s\n" % new_list_xiangguan[i])
            self.assertEqual(old_list_xiangguan[i], new_list_xiangguan[i], msg=u"相关性匹配错误")
            f.write("-----------------------------------------\n")
...
if __name__ == "__main__":
    ...
    runner = HTMLTestRunner.HTMLTestRunner(
        stream=fp,
        title=u'搜索页计算模块--百货query&未预测分类query新老产品版本结果比对',
        description=u'针对所有预测为百货的query和未预测出分类的query，对比新老产品的同一版本的结果是否一致，'
