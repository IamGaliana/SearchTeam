...
public class XMLParser {
	...
	private static String XPATH_TOTALCNT_DXB="//TotalCnt";
	private static String XPATH_STOCK_STATUS_DXB = "//stock_status";
	private static String XPATH_DD_SELL_DXB = "//is_dd_sell";
	private static String XPATH_PRODUCT_MEDIU_DXB="//product_medium";
	private static String XPATH_ORIGINAL_QUERY="//original_query";
	...
	private static String XPATH_ORIGINALQUERY = "//result/StatInfo/mix_reco/original_query";
	private static String XPATH_FIXEDQUERY = "//result/StatInfo/mix_reco/fixed_query";
	...
	private static String XPATH_INDEX_NAME = "name";
	private static String XPATH_INDEX_FULLTM = "full_tm";
	private static String XPATH_INDEX_INCNUM = "inc_num";
	
	
	/** 
	 * 解析 http://10.255.254.188:8390/?_show_module=1 的结果，获取模块的full_tm
	 * <result>
			<module name="searcher" full_tm="1438008683" inc_num="352776" static_doc_cnt="16286546" total_doc_cnt="16288361"/>
			<module name="list_ranking" full_tm="1438586280" inc_num="0" static_doc_cnt="0" total_doc_cnt="0"/>
			...
		</result>
	 * @param doc				document
	 * @param module_name 		想要获取的模块名称
	 * @return					指定module_name的full_tm值
	 */
	public static String Index_fulltm(Document doc, String module_name){
		String value = "";
		try{
			Element root = doc.getRootElement();			// result
			List<Element> childElements = root.elements();	// module
			for(Element child : childElements){				// traverse modules
				if(child.attributeValue(XPATH_INDEX_NAME).equals(module_name)){	// in module，if(name == searcher),return full_tm value
					value = child.attributeValue(XPATH_INDEX_FULLTM);
					break;
				}
			}
			return value;
	...
	/**
	 * 解析 http://10.255.254.188:8390/?_show_module=1 的结果，获取模块的inc_num
	 * @param doc			document
	 * @param module_name	specified module 		
	 * @return				inc num of specified module
	 */
	public static String Index_IncNum(Document doc, String module_name){
		String value = "";
		try{
			Element root = doc.getRootElement();			// result
			List<Element> childElements = root.elements();	// module
			for(Element child : childElements){				// traverse modules
				if(child.attributeValue(XPATH_INDEX_NAME).equals(module_name)){	// in module，if(name == searcher),return full_tm value
					value = child.attributeValue(XPATH_INDEX_INCNUM);
					break;
				}
			}
			return value;
	...
	public static String PageCount(Document doc) {
		String value = "0";
		try{		
			if(!doc.selectNodes(XPATH_PAGE_COUNT).isEmpty()){
	...
	public static String PageIndex(Document doc) {
		String value = "0";
		try{		
			if(!doc.selectNodes(XPATH_PAGE_INDEX).isEmpty()){
...
//		logger.debug("- [XMLParser] - Parse the xml! "/* + xml */);
	...
	public static String getTotalcnt(Node node) {
		String value;
		try {
			Node subNode = (Node) node.selectNodes(XPATH_TOTALCNT_DXB).get(0);
			value = subNode.getStringValue();
		} catch (Exception e) {			
			value = "0";
		}
	 ...
	 * 获得某个产品是否当当自营
	 */
	public static String product_dd_sell_dxb(Node node) {
		String value;
		try {
			Node subNode = (Node) node.selectNodes(XPATH_DD_SELL_DXB).get(0);
			value = subNode.getStringValue();
		} catch (Exception e) {
			value = "0";
		}
	 ...
	 * 获得某个产品是否是图书
	 */
	public static String product_medium_dxb(Node node) {
		String value;
		try {
			Node subNode = (Node) node.selectNodes(XPATH_PRODUCT_MEDIU_DXB).get(0);
			value = subNode.getStringValue();
		} catch (Exception e) {
			value = "0";
		}
...
//		tm.put(XPATH_DEVICE_ID, get_product_info(node, XPATH_DEVICE_ID));
	...
	public static String getSingleNode(Node node,String xpath){
		String value;
		try {
			Node subNode = (Node) node.selectNodes(xpath).get(0);
			value = subNode.getStringValue();
		} catch (Exception e) {			
			value = "0";
		}
	...
	public static String prodSpecifiedAttrib(Node node, String attribName){
		String value;
		try {
			Node subNode = (Node) node.selectNodes(attribName).get(0);
			value = subNode.getStringValue();
		} catch (Exception e) {
			value = "";
		}
	 ...
	 * @return 智能纠错前的query词
	 */
	public static String getOriginalQuery(Document doc){
		String value = "";
		try{
			Node subnode = (Node)doc.selectNodes(XPATH_ORIGINALQUERY).get(0);
			value = subnode.getStringValue();
		}
		catch(Exception e){
			value = "";
	 ...
	 * @return 智能纠错后的query词
	 */
	public static String getFixedQuery(Document doc){
		String value = "";
		try{
			Node subnode = (Node)doc.selectNodes(XPATH_FIXEDQUERY).get(0);
			value = subnode.getStringValue();
		}
		catch(Exception e){
			value = "";
