...
public class AssertTools {
	...
	public static boolean singleTopStatus(String score) {		
		int iscore = Integer.valueOf(score);
		if (iscore>=268435456) { // 只有第29位（单品）=1，其他为均为0时，十进制数=268435456
			return true;
		}else {
			return false;
		}			
	 ...
	 * @return 文本相关性得分
	 */
	public static int BusinessScore(Integer score) {

		//int iscore = Integer.valueOf(score);
		// 转成二进制
		String b_scope_full = Integer.toBinaryString(score);
		// 拿到商业因素的得分
		String text_rel_full = b_scope_full.substring(b_scope_full.length() - 8, b_scope_full.length());
		// 从二进制重新转换成10进制
		int tr = Integer.parseInt(text_rel_full, 2);
		return tr;
	}
	...
	// 调试使用
		public static void main(String[] args) {
			Integer aString = 247513469;
			
			System.out.println(BusinessScore(aString));
