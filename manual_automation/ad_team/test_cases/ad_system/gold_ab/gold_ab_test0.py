...
class GoldAB0(unittest.TestCase):
    def setUp(self):
        self.url0 = "http://10.3.255.91:9080/getAds/cpc/0/0/4/%s/184537119/0/0/0/107169205/2/-/0/0/0/debug"
        self.url500 = "http://10.3.255.91:9080/getAds/cpc/500/0/4/%s/184537119/0/0/0/107169205/2/-/0/0/0/0/-/debug"
    ...
    def test_gold_ab(self, query):
        ...
        searchtype_f0, m_oriscore_list0, ad_id0, gold_search_type_list0, ad_id_all0, gold_search_type_all0 = goldab_get_values(ad_data0)
        print "ad_id_all0", ad_id_all0
        print "searchtype0", searchtype_f0
        print "m_oriscore_list0", m_oriscore_list0
        print "gold_search_type_list0", gold_search_type_list0

        if len(ad_data0) < 4:
            for i in xrange(len(ad_data0)):
                self.assertEqual(ad_data0[i]['gold_search_type'], '0', '返回广告个数小于4时，结果0中的gold_search_type不都为0')
            self.assertEqual(len(ad_result500), 0, '返回广告个数小于4时，结果500不为空')
        else:
            if len(match_result0) < 3:
                ...
                if '0' in m_oriscore_list0:
                    for i in gold_search_type_list0:
                    #print "i", i
                    #print "type(i)", type(i)
                        self.assertEqual(i, '0', '请求posid为0返回结果中有m_oriscore=0的，所有gold_search_type不都为0')
                    self.assertEqual(len(ad_result500), 0, '请求posid为0返回符合条件结果大于等于3时，您的500的结果中广告的个数不=0')
                else:
                    self.assertNotEqual(len(ad_result500), 0, '请求posid为0返回符合条件结果大于等于3时，您的500的结果中广告的个数不应=0')
...
if __name__ == "__main__":
    ...
    runner = HTMLTestRunner.HTMLTestRunner(
        ...
        5) 请求posid为0返回结果中有m_oriscore=0的，则所有gold_search_type应都为0
        '''
    )
