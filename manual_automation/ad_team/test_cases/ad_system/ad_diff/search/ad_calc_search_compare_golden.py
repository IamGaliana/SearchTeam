...
query_list = []
tables = excel_table_byindex(KEYWORD_FILE, by_index = 3)   #读取第4个sheet的数据
for row in tables:
    query_list.append(row["keyword"].encode("utf-8"))

#query_list = ['衬衫','短裤']
title = time.strftime('%Y-%m-%d_%H.%M.%S_',time.localtime(time.time()))
test_report = REPORT_PATH + title + u"ad_calc_search_compare_golden.html"
test_log = LOG_PATH + title + u"ad_calc_search_compare_golden.txt"
...
class ad_calc_search(unittest.TestCase):
    def setUp(self):
        self.URL_NEW = "http://%s/getAds/cpc/500/0/100/%s/184537119/0/0/0/107169205/%s/-/0/0/0/debug"
        self.URL_OLD = "http://%s/getAds/cpc/500/0/100/%s/184537119/0/0/0/107169205/%s/-/0/0/0/debug"
        pass
    @data(*query_list)
    def test_case(self, query):
        ...
        old_result_x = run_for_search_category(old_url)     #字典，旧版本的所有数据的字典list，包含ad_id，DebugAdInfo，debug_info，bid_ctr，score，search_type， 第二个返回结果为请求预测类型
        new_result_x = run_for_search_category(new_url)     #字典，新版本的所有数据字典list，包含ad_id，DebugAdInfo，debug_info，bid_ctr，score，search_type， 第二个返回结果为请求预测类型
        ...
        if not debugquerypath.startswith("58"):
...
if __name__ == "__main__":
    ...
    runner = HTMLTestRunner.HTMLTestRunner(
        stream=fp,
        title=u'搜索页计算模块---黄金展位, version = %s' % version,
