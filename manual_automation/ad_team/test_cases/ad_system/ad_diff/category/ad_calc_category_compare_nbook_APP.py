...
test_report = u"\\\\ddfile\\技术部\\测试部\\广告系统\\自动化测试报告\\category\\"+title+u"ad_calc_category_compare_nbook_APP.html"
test_log = u"\\\\ddfile\\技术部\\测试部\\广告系统\\自动化测试报告\\category\\"+title+u"ad_calc_category_compare_nbook_APP.txt"
...
class ad_calc_category(unittest.TestCase):
    def setUp(self):
        self.URL_NEW = "http://10.3.255.91:9080/getAds/cpc/201/0/50/%s/3232262672/20141022092715613302051844096892271/0/0/0/1/%s/0/0/0/0/debug"
        self.URL_OLD = "http://10.3.255.207:9080/getAds/cpc/201/0/50/%s/3232262672/20141022092715613302051844096892271/0/0/0/1/%s/0/0/0/0/debug"
    ...
    def test_case(self, category_id):
        ...
        if len(old_ad_info_list) == 0 and len(new_ad_info_list) == 0:
            raise unittest.SkipTest("no ids")
...
if __name__ == "__main__":
    ...
    runner = HTMLTestRunner.HTMLTestRunner(
        stream=fp,
        title=u'列表页计算模块---百货---APP, version = %s' % version,
