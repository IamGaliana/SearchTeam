...
def run_for_search_category(url):
    try:
        ...
        result = []
    try:
        DebugQueryPath = response["debug"]['DebugQueryPath']
    except Exception, ex:
        DebugQueryPath = '-1'
    list = []
    for i in range(0, len(result)):
        ...
        t_akscore = result[i]["m_score"]
        zidian["ad_id"] = result[i]["ad_id"]
        zidian["ad_user"] = result[i]["ad_user"]
        zidian["cost"] = result[i]["cost"]
        zidian["product_id"] = result[i]["product_id"]
        zidian["cat_id"] = result[i]["cat_id"]
        zidian["pos"] = result[i]["pos"]
        zidian["extra_json"] = result[i]["extra_json"]
        zidian["gold_search_type"] = result[i]["gold_search_type"]
        zidian["bid"] = bid
        zidian["sim"] = result[i]["sim"]
        zidian["ctr"] = ctr
        zidian["score"] = result[i]["score"]
        zidian["related_type"] = result[i]["related_type"]
        zidian["t_akscore"] = t_akscore
        zidian["cat_path"] = result[i]["cat_path"]
        zidian["match"] = match
        zidian["bid_ctr"] = str(bid)+str(ctr)
        zidian["match_bid_ctr"] = str(match)+str(bid)+str(ctr)
        zidian["bid*akscore"] = int(bid) * int(t_akscore)
        zidian["search_type"] = result[i]["precise"]
...
#  读取搜索计算模块，返回存有ad_id，debug_info
#  bid_dtr的字典组成的list，和ad_id的set
#  list 和本次请求的DebugQueryPath、DebugC2CPath, DebugSynPath, DebugSynC2CPath信息     #
##########################################
'''
def run_for_search_category_alldebug(url):
    try:
        response = response_ad_calc_server(url)
        result = response['data']
    except Exception, ex:
    ...
    try:
        DebugQueryPath = response["debug"]['DebugQueryPath']
        DebugC2CPath = response["debug"]['DebugC2CPath']
        DebugSynPath = response["debug"]['DebugSynPath']
        DebugSynC2CPath = response["debug"]['DebugSynC2CPath']
    except Exception, ex:
        DebugQueryPath = '-1'
        DebugC2CPath = '-1'
        DebugSynPath = '-1'
    ...
    for i in range(0, len(result)):
        ...
        zidian["bid*akscore"] = int(bid) * int(t_akscore)
        zidian["search_type"] = result[i]["precise"]
        zidian["app_sim_flag"] = result[i]["app_sim_flag"]
        list.append(zidian)
    return list, DebugQueryPath, DebugC2CPath, DebugSynPath, DebugSynC2CPath