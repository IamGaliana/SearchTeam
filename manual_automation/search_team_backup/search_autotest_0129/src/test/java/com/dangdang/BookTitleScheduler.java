...
import com.dangdang.verifier.analysis.ClickPositionAnalysis;
//import com.dangdang.verifier.booktitle.*;

import com.dangdang.util.DBConnUtil;

public class BookTitleScheduler {

	private static Logger logger = Logger.getLogger(BookTitleScheduler.class);
	...
	@AfterClass
	public void clearup() {
		logger.fatal(" - [LOG_RESULT] - " + "算数平均点击位置变化为：" + sum / count);
		logger.fatal("*******************************************************");
		int pre_p0 = 0;
		int pre_p1 = 0;
		int pre_p2 = 0;
		...
		logger.fatal("*	旧版本位置0～4:   " + pre_p0 / pre_pSum * 100 + "%	*	新版本位置0～4:   " + act_p0 / act_pSum * 100 + "%	*");
		logger.fatal("*	旧版本位置4～12:   " + pre_p1 / pre_pSum * 100 + "%	*	新版本位置4～12:   " + act_p1 / act_pSum * 100 + "%	*");
		logger.fatal("*	旧版本位置12～60:   " + pre_p2 / pre_pSum * 100 + "%	*	新版本位置12～60:   " + act_p2 / act_pSum * 100 + "%	*");
		logger.fatal("*	旧版本位置60～:   " + pre_p3 / pre_pSum * 100 + "%	*	新版本位置60～:   " + act_p3 / act_pSum * 100 + "%	*");
		logger.fatal("*******************************************************");
