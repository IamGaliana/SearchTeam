...
public class ListRankingBlackListVerifier{
	...
	// 日志记录器
	private static Logger logger = Logger.getLogger(ListRankingBlackListVerifier.class);
	...
	public boolean doVerify(BlackListQuery category) {
	    try {
		    ...
		    if(start_date.before(now)&&end_date.after(now)){//黑名单生效
		    	logger.fatal("blacklist filer category ["+category.getBlackId()+"] is 生效");
		    	return isBlackList(category,true);
		    }else{//黑名单不生效
		    	logger.fatal("blacklist filer category ["+category.getBlackId()+"] is 不生效");
