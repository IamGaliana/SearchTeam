...
//							logger.fatal(" - [LOG_SUCCESS] - "+"recommend product is prefect!!!!");
//							return true;
//						}else{
...
//				}
		    //未进入垂搜
				...
				if(isNullOrEmpty(query.getQuery())||isNullOrEmpty(query.getShop_id())||isNullOrEmpty(query.getBrand_id())
					...
					logger.fatal(" - [LOG_SUCCESS] - "+"The required fields is missing");
					return true;
				}

				//如果设置中招商品就少于5个，该设置不生效
				if(query_products.size()<5){
					logger.fatal(" - [LOG_SUCCESS] - "+"The number of products in query setting is less then 5");
					logger.fatal(" - [LOG_SUCCESS] - "+"products in query setting:"+query.getShop_products());
					return true;
				}
				else{
					//得到有效的招商品个数
