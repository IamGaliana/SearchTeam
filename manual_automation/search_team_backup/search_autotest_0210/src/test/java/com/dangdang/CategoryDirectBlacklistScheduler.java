	...
	public void CategoryDirectBlacklist() { 
			...
			for(FuncQuery query : list){
				...
				if(CategoryDirectBlacklistVerifier.Verify(query)){
					logger.fatal(" - [PASSED] - "+"CategoryDirect Blacklist for:"+query.getFquery());
					pass++;
				}else{
					logger.fatal(" - [FAILED] - "+"CategoryDirect Blacklist for:"+query.getFquery());
					fail++;
				}
				}catch(Exception e){
	...
	public void CategoryFeedbackFilter() { 
			...
			for(FuncQuery query : list){
				...
				if(CategoryFeedbackFilterVerifier.Verify(query)){
					logger.fatal(" - [PASSED] - "+"CategoryFeedback Filter for:"+query.getFquery());
					pass++;
				}else{
					logger.fatal(" - [FAILED] - "+"CategoryFeedback Filter for:"+query.getFquery());
					fail++;
				}
				}catch(Exception e){
					 ByteArrayOutputStream buf = new ByteArrayOutputStream();
