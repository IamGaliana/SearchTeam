...
public class GSearchScheduler {
	private static Logger logger = Logger.getLogger(GSearchScheduler.class);	
	{ 		
		PropertyConfigurator.configure("conf/gSearch_log4j.properties");
	}
...
//			tl.start("id between 727 and 766", "fvp_id in (0)");
			...
			logger.fatal("eslipse for gSearch_book_search:"+(d2-d)/1000.0);
		} catch (Exception e) {
...
//			tl.start("id between 767 and 806", "fvp_id in (0)");
			long d2 = System.currentTimeMillis();
			logger.fatal("eslipse for gSearch_book_list:"+(d2-d)/1000.0);
		} catch (Exception e) {
			ByteArrayOutputStream baos = new ByteArrayOutputStream();  
			e.printStackTrace(new PrintStream(baos));  
			String exception = baos.toString();  
...
//			tl.start("id between 807 and 827", "fvp_id in (0)");
			long d2 = System.currentTimeMillis();
			logger.fatal("eslipse for gSearch_book_merge:"+(d2-d)/1000.0);
		} catch (Exception e) {
			ByteArrayOutputStream baos = new ByteArrayOutputStream();  
			e.printStackTrace(new PrintStream(baos));  
			String exception = baos.toString();  
...
//			tl.start("id between 828 and 867", "fvp_id in (0)");
			long d2 = System.currentTimeMillis();
			logger.fatal("eslipse for gSearch_ebook_search:"+(d2-d)/1000.0);
		} catch (Exception e) {
			ByteArrayOutputStream baos = new ByteArrayOutputStream();  
			e.printStackTrace(new PrintStream(baos));  
			String exception = baos.toString();  
...
//			tl.start("id between 868 and 907", "fvp_id in (0)");
			long d2 = System.currentTimeMillis();
			logger.fatal("eslipse for gSearch_ebook_list:"+(d2-d)/1000.0);
		} catch (Exception e) {
			ByteArrayOutputStream baos = new ByteArrayOutputStream();  
			e.printStackTrace(new PrintStream(baos));  
			String exception = baos.toString();  
...
//			tl.start("id between 908 and 926", "fvp_id in (0)");
			long d2 = System.currentTimeMillis();
			logger.fatal("eslipse for gSearch_ebook_merge:"+(d2-d)/1000.0);
		} catch (Exception e) {
			ByteArrayOutputStream baos = new ByteArrayOutputStream();  
			e.printStackTrace(new PrintStream(baos));  
			String exception = baos.toString();  
...
//			tl.start("id between 1229 and 1328", "1=2");
			long d2 = System.currentTimeMillis();
			logger.fatal("eslipse for gSearch_digital:"+(d2-d)/1000.0);
		} catch (Exception e) {
			ByteArrayOutputStream baos = new ByteArrayOutputStream();  
			e.printStackTrace(new PrintStream(baos));  
			String exception = baos.toString();  
...
//			tl.start("id between 1329 and 1426", "1=2");
			long d2 = System.currentTimeMillis();
			logger.fatal("eslipse for gSearch_music:"+(d2-d)/1000.0);
		} catch (Exception e) {
			ByteArrayOutputStream baos = new ByteArrayOutputStream();  
			e.printStackTrace(new PrintStream(baos));  
			String exception = baos.toString();  
...
//			tl.start("id between 1427 and 1526", "1=2");
			long d2 = System.currentTimeMillis();
			logger.fatal("eslipse for gSearch_video:"+(d2-d)/1000.0);
		} catch (Exception e) {
			ByteArrayOutputStream baos = new ByteArrayOutputStream();  
			e.printStackTrace(new PrintStream(baos));  
			String exception = baos.toString();  
...
//			tl.start("id between 1530 and 1760", "1=2");
			long d2 = System.currentTimeMillis();
			logger.fatal("eslipse for gSearch_b2c:"+(d2-d)/1000.0);
		} catch (Exception e) {
			ByteArrayOutputStream baos = new ByteArrayOutputStream();  
			e.printStackTrace(new PrintStream(baos));  
			String exception = baos.toString();   
