	...
	public boolean Verifier(ProdIterator iterator, Map<String, String> map) {
		...
		if (iterator.getTotalCount() < 1) {
			...
			for (Entry<Integer, List<Node>> entry : resultmap.entrySet()) {
				logger.info("配置各分类商品情况："+categorys[entry.getKey()].toString()+entry.getValue().toString());
			}
...
//				logger.info("hehe");
	...
	private boolean isvalidProd(Integer integer, Node node) {
		...
		return AssertTools.HasImages(scoreString)&&AssertTools.StockStatus(scoreString)&&(AssertTools.TextRelation(scoreString)>=5);
	}
...
//			logger.info(" - 该分类无商品");
			...
			for (int i = 0; i < value.size(); i++) {
				if (isvalidProd(integer, value.get(i))) {
					...
					logger.info("该分类下前"+String.valueOf(i+1)+"个商品为新品");
